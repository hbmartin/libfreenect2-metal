    ...
    if controller.sensors[0].positive:
        try:
            ...
            if len(msg_raw) == 4:
                ...
                frame_raw = b""
                for _ in range(nb_of_packet_per_frame):
                ...
                frame = cv2.imdecode(
                    numpy.fromstring(frame_raw, dtype=numpy.uint8), cv2.IMREAD_GRAYSCALE
                )
                ...
                if frame is not None:
                    ...
                    flat_frame = list(itertools.chain(*frame_rows))

                    # image_buffer = bgl.Buffer(bgl.GL_INT, [width*height*3], flat_frame)
                    image_buffer = bgl.Buffer(bgl.GL_BYTE, width * height, flat_frame)
