...
template <typename T> struct FeatureSelectorRecipient
{
  static uint8_t get();
};

template <> struct FeatureSelectorRecipient<DeviceFeatureSelector>
{
  static uint8_t get() { return LIBUSB_RECIPIENT_DEVICE; };
};

template <> struct FeatureSelectorRecipient<InterfaceFeatureSelector>
{
...
int set_isochronous_delay(libusb_device_handle* handle, int timeout)
{
  ...
  return libusb_control_transfer(handle, bmRequestType, bRequest, wValue, wIndex, data, wLength,
                                 timeout);
}

int set_sel(libusb_device_handle* handle, int timeout, uint8_t u1sel, uint8_t u1pel, uint8_t u2sel,
            uint8_t u2pel)
{
...
int set_feature_function_suspend(libusb_device_handle* handle, int timeout, bool low_power_suspend,
                                 bool function_remote_wake)
{
  ...
  if (r == LIBUSB_SUCCESS)
  {
    for (int interface_idx = 0; interface_idx < config_desc->bNumInterfaces; ++interface_idx)
    {
      ...
      if (interface.num_altsetting > alternate_setting)
      {
        ...
        {
          if (interface_desc.endpoint[endpoint_idx].bEndpointAddress == endpoint &&
              (interface_desc.endpoint[endpoint_idx].bmAttributes & 0x3) ==
        ...
        if (endpoint_desc != 0)
        {
          ...
          // ctx is only used for error reporting, libusb should better ask for a libusb_device
          // anyway...
          r = libusb_get_ss_endpoint_companion_descriptor(NULL /* ctx */, endpoint_desc,
                                                          &companion_desc);
...
UsbControl::UsbControl(libusb_device_handle* handle) : handle_(handle), timeout_(1000) {}

UsbControl::~UsbControl() {}

#define CHECK_LIBUSB_RESULT(__CODE, __RESULT)                                                      \
  if (((__CODE) = ((__RESULT) == LIBUSB_SUCCESS ? Success : Error)) == Error)                      \
  LOG_ERROR
UsbControl::ResultCode UsbControl::setConfiguration()
{
  ...
  CHECK_LIBUSB_RESULT(code, r) << "failed to get configuration! "
  ...
  if (code == Success)
  {
    if (current_config_id != desired_config_id)
    {
      ...
      CHECK_LIBUSB_RESULT(code, r)
          << "failed to set configuration! " << usb::formatLibusbError(r, true);
    }
  }
...
UsbControl::ResultCode UsbControl::releaseInterfaces()
{
  ...
  CHECK_LIBUSB_RESULT(code, r) << "failed to release interface with ControlAndRgbInterfaceId(="
                               << ControlAndRgbInterfaceId << ")! "
                               << usb::formatLibusbError(r, true);
  ...
  if (code == Success)
  {
    ...
    CHECK_LIBUSB_RESULT(code, r) << "failed to release interface with IrInterfaceId(="
                                 << IrInterfaceId << ")! " << usb::formatLibusbError(r, true);
  }
...
UsbControl::ResultCode UsbControl::setIsochronousDelay()
{
  ...
  CHECK_LIBUSB_RESULT(code, r) << "failed to set isochronous delay! "
                               << usb::formatLibusbError(r, true);
  return code;
}
...
UsbControl::ResultCode UsbControl::setPowerStateLatencies()
{
  ...
  UsbControl::ResultCode code;
  CHECK_LIBUSB_RESULT(code, r) << "failed to set power state latencies! "
                               << usb::formatLibusbError(r, true);
  return code;
}
...
UsbControl::ResultCode UsbControl::enablePowerStates()
{
  ...
  CHECK_LIBUSB_RESULT(code, r) << "failed to enable power states U1! "
                               << usb::formatLibusbError(r, true);

  if (code == Success)
  {
    r = libusb_ext::set_feature(handle_, timeout_, libusb_ext::U2_ENABLE);
    CHECK_LIBUSB_RESULT(code, r) << "failed to enable power states U2! "
                                 << usb::formatLibusbError(r, true);
  }
...
UsbControl::ResultCode UsbControl::setVideoTransferFunctionState(UsbControl::State state)
{
  ...
  UsbControl::ResultCode code;
  CHECK_LIBUSB_RESULT(code, r) << "failed to set video transfer function state! "
                               << usb::formatLibusbError(r, true);
  return code;
}
...
UsbControl::ResultCode UsbControl::setIrInterfaceState(UsbControl::State state)
{
  ...
  UsbControl::ResultCode code;
  CHECK_LIBUSB_RESULT(code, r) << "failed to set ir interface state! "
                               << usb::formatLibusbError(r, true);
  return code;
}
...
UsbControl::ResultCode UsbControl::getIrMaxIsoPacketSize(int& size)
{
  ...
  UsbControl::ResultCode code;
  CHECK_LIBUSB_RESULT(code, r) << "failed to get max iso packet size! "
                               << usb::formatLibusbError(r, true);
  return code;
}
